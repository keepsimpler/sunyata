...
\title{
  A Deep Bayesian Inference Architecture  % coupled to neural networks
  % Bayesian inference and language modeling
  }
...
From a probabilistic perspective, we can view the task of supervised learning as fitting a conditional model of the form $p(\bm{y}|\bm{x})$, which specifies a distribution over outputs given inputs (P44 of \cite{murphy2022probabilistic}).
...
The posterior is a kind of weighted average (mean) of the prior, where the likelihood are the weights, the evidence are the summation of the weights. 

At each Bayesian iteration, the likelihood is provided by the output of the corresponding layer of the neural network.
...
\subsection{Deep Bayesian Inference Architecture}

\paragraph*{Advantage} Similar to auxiliary loss, DBI reduce the vanishing gradient problem for earlier layers, stabilize the training and is used as regularization.
