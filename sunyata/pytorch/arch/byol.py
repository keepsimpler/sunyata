...
class NetWrapper(nn.Module):
    def __init__(self, net:nn.Module, projection_size, projection_hidden_size, layer=-2, use_simsiam_mlp=False):
        super().__init__()
        self.net = net
        self.layer = layer
...
class BYOL(nn.Module):
    def __init__(
        ...
        # get device of network and mask wrapper same device
    ...
    def forward(
        ...
        assert not (self.training and x.shape[0] == 1), 'you must have greater than 1 sample when training, due to the batch-norm in the projection layer'
