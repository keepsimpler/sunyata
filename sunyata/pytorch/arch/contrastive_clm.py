...
from sunyata.pytorch.arch.loss import InfoNCE, ECELoss, BarlowTwins
...
class ContrastiveCLMCov(BaseModule):
    def __init__(self, cfg:ContrastiveCLMCfg):
        ...
        if cfg.temperature is None:
            self.temperature = (cfg.seq_len - 1) ** 0.5
        else:
            self.temperature = cfg.temperature
        self.loss_fn = BarlowTwins(temperature=self.temperature)
